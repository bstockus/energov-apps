...
-- exec rpt_PM_Permit_Detailed_Dept_A '7d2a81d0-40bb-49cd-a431-c2b223fc5299',''
CREATE PROCEDURE rpt_PM_Permit_Detailed_Dept_A
...
SELECT PM.PMPERMITID, PM.PERMITNUMBER, PM.ISSUEDATE, PM.EXPIREDATE, PM.VALUE AS VALUATION, 
	...
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Municipality_Name') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Name')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Municipality_Name') END Municipality_Name,
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Municipality_Address') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Address')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Municipality_Address') END Municipality_Address,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Notice_Prefix') Permit_Notice_Prefix,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Page_Footer') Permit_Page_Footer,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Signature_1') Permit_Signature_1,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Signature_2') Permit_Signature_2,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Permit_Detailed_Dept_A_Inspection_Phone') Inspection_Phone,
	(SELECT R.[IMAGE] FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'Permit_Detailed_Dept_A_Logo') LOGO,
	(SELECT CASE WHEN R.[IMAGE] IS NULL  THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'Permit_Detailed_Dept_A_Logo') SHOWLOGO,
