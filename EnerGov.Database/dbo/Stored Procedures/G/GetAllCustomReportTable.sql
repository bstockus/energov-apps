
CREATE PROCEDURE [dbo].[GetAllCustomReportTable]
       (@ids AS IdTableType READONLY)
AS
BEGIN
      SET NOCOUNT ON;
    
      DECLARE @queries TABLE
              ([CustomTable] VARCHAR(28)
              ,[LayoutId] CHAR(36)
              ,[Ids] NVARCHAR(MAX));

      INSERT    INTO @queries
                ([CustomTable]
                ,[LayoutId]
                ,[Ids])
      SELECT    [eclr].[CustomTable]
               ,[eclr].[LayoutId]
               ,'(' + STUFF((SELECT ',''' + [eclr2].[Id] + ''''
                             FROM   [EntityCustomLayoutRef] AS [eclr2]
                             WHERE  [eclr2].[LayoutId] = [eclr].[LayoutId]
                                    AND [eclr2].[CustomTable] = [eclr].[CustomTable]
                                    AND [eclr2].[Id] IN (SELECT Id
                                                         FROM   @ids)
                FOR         XML PATH('')
                               ,ROOT('strings')
                               ,TYPE).value('/', 'NVarchar(MAX)'), 1, 1, '') + ')' AS [Ids]
      FROM      (
                 SELECT DISTINCT
                        [CustomTable]
                       ,[LayoutId]
                 FROM   [dbo].[EntityCustomLayoutRef]
                 WHERE  [Id] IN (SELECT [Id]
                                 FROM   @ids)
                ) AS [eclr];
                
      IF OBJECT_ID('tempdb..#vals') IS NOT NULL
         DROP TABLE #vals;
	   
      CREATE TABLE #vals
             ([Id] CHAR(36)
             ,[Field] NVARCHAR(4000)
             ,[Value] NVARCHAR(MAX));

	 /* declare variables */
      DECLARE @query NVARCHAR(MAX);
	 
      DECLARE customFieldCursor CURSOR FAST_FORWARD READ_ONLY
      FOR
      SELECT    [query].[query]
      FROM      @queries [q]
      OUTER APPLY (
                   SELECT   'INSERT INTO #vals([Id], [Field], [Value])
SELECT [Id], [Field], [Value]
FROM (SELECT [pm1].[ID] as [Id]' + [s].[s].query('/strings/SelectSection').value('/', 'Nvarchar(max)') + '
	   FROM [dbo].[' + [q].[CustomTable] + '] [pm1] 
	   JOIN [dbo].[' + [q].[CustomTable] + '2] [pm2] ON [pm1].[ID] = [pm2].[ID]
	   WHERE [pm1].[ID] IN ' + [q].[Ids] + ') as [p] 
UNPIVOT (Value FOR Field IN (' + STUFF([s].[s].query('/strings/UnpivotSection').value('/', 'Nvarchar(max)'), 1, 1, '') + ')) as [unpvt]' AS query
                   FROM     (
                             SELECT (
                                     SELECT ', CONVERT(NVARCHAR(4000),' + CASE WHEN [obj].[FKCUSTOMFIELDLAYOUTCONTROLTYPE] IN (1) THEN '[' + [c2].[SFIELDNAME] + ']'
                                                                               WHEN [obj].[FKCUSTOMFIELDLAYOUTCONTROLTYPE] = 2
                                                                                    AND [pickList].[BALLOWMULTIPLESELECTIONS] = 0
                                                                               THEN '(SELECT TOP 1 [c].[SVALUE] FROM [CUSTOMFIELDPICKLISTITEM] AS [c] WHERE [c].[FKGCUSTOMFIELDPICKLIST] = '''
                                                                                    + [pickList].[GCUSTOMFIELDPICKLIST]
                                                                                    + ''' AND [c].[GCUSTOMFIELDPICKLISTITEM] = [' + [c2].[SFIELDNAME] + '])'
                                                                               WHEN [obj].[FKCUSTOMFIELDLAYOUTCONTROLTYPE] = 2
                                                                                    AND [pickList].[BALLOWMULTIPLESELECTIONS] = 1
                                                                               THEN '[dbo].[GetMultiSelectValue](pm1.[ID],''' + [c2].[GCUSTOMFIELD] + ''')'
                                                                               WHEN [obj].[FKCUSTOMFIELDLAYOUTCONTROLTYPE] = 3
                                                                               THEN 'CASE WHEN COALESCE([' + [c2].[SFIELDNAME] + '],0) = 0 THEN ''No'' ELSE ''Yes'' END'
                                                                          END + ') as [' + CASE WHEN [obj].[ISLABELSURPRESSED] = 1 THEN [c2].[SCUSTOMFIELD]
																									+ ISNULL(NULLIF(CONVERT(VARCHAR(32), ROW_NUMBER() OVER (PARTITION BY [c2].[SCUSTOMFIELD] ORDER BY [c2].[GCUSTOMFIELD])), --ATC 5/11/18 - Added this section to ensure unique values returned.  obj.label already had this code when updated.
																													 '1'),'')
                                                                                                ELSE [obj].[SLABEL]
                                                                                                     + ISNULL(NULLIF(CONVERT(VARCHAR(32), ROW_NUMBER() OVER (PARTITION BY [obj].[SLABEL] ORDER BY [c2].[GCUSTOMFIELD])),
                                                                                                                     '1'), '')
                                                                                           END + ']' AS [SelectSection]
                                           ,',[' + CASE WHEN [obj].[ISLABELSURPRESSED] = 1 THEN [c2].[SCUSTOMFIELD]
															+ ISNULL(NULLIF(CONVERT(VARCHAR(32), ROW_NUMBER() OVER (PARTITION BY [c2].[SCUSTOMFIELD] ORDER BY [c2].[GCUSTOMFIELD])), --ATC 5/11/18 - Added this section to ensure unique values returned.  obj.label already had this code when updated.
																			 '1'),'')
                                                        ELSE [obj].[SLABEL]
                                                             + ISNULL(NULLIF(CONVERT(VARCHAR(32), ROW_NUMBER() OVER (PARTITION BY [obj].[SLABEL] ORDER BY [c2].[GCUSTOMFIELD])),
                                                                             '1'), '')
                                                   END + ']' AS [UnpivotSection]
                                     FROM   [CUSTOMFIELDLAYOUT] AS [c]
                                     JOIN   [CUSTOMFIELDOBJECT] AS [obj]
                                            ON [obj].[FKGCUSTOMFIELDLAYOUT] = [c].[GCUSTOMFIELDLAYOUTS]
                                     LEFT JOIN [CUSTOMFIELDOBJECT] AS [tab]
                                            ON [tab].[GCUSTOMFIELD] = [obj].[FKGPARENTCONTROL]
                                     JOIN   [CUSTOMFIELD] AS [c2]
                                            ON [c2].[GCUSTOMFIELD] = [obj].[GCUSTOMFIELD]
                                     LEFT JOIN [CUSTOMFIELDPICKLIST] AS [pickList]
                                            ON [pickList].[FKGCUSTOMFIELD] = [c2].[GCUSTOMFIELD]
                                     WHERE  [c].[GCUSTOMFIELDLAYOUTS] = [q].[LayoutId]
                                            AND [c2].[SFIELDNAME] != ''
                                            AND [obj].[FKCUSTOMFIELDLAYOUTCONTROLTYPE] IN (1, 2, 3)
                                     ORDER BY [tab].[TABINDEX]
                                           ,[obj].[TABINDEX]
                                           ,[obj].[FTOPPOS]
                                           ,[obj].[FLEFTPOS]
                                    FOR
                                     XML PATH('')
                                        ,ROOT('strings')
                                        ,TYPE
                                    ) AS [s]
                            ) AS [s]
                  ) AS query;
	 
      OPEN customFieldCursor;
	 
      FETCH NEXT FROM customFieldCursor INTO @query;
	 
      WHILE @@FETCH_STATUS = 0
            BEGIN

			   --PRINT @query
                  EXEC [sys].[sp_executesql] @stmt = @query;

                  FETCH NEXT FROM customFieldCursor INTO @query;
            END;
	 
      CLOSE customFieldCursor;
      DEALLOCATE customFieldCursor;

      SELECT    [i].[Id]
               ,[v].[Field]
               ,[v].[Value]
      FROM      @ids AS [i]
      JOIN #vals AS [v]
                ON [v].[Id] = [i].[Id];

END;
