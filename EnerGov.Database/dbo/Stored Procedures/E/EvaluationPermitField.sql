...
CREATE PROCEDURE [dbo].[EvaluationPermitField]
...
BEGIN
	...
	DECLARE @CustomSaverPermitManagement nvarchar(50) = 'CUSTOMSAVERPERMITMANAGEMENT'	
	...
	BEGIN											
		...
		IF RTRIM(LTRIM(@TableName)) = 'PMPERMIT' --This is permit fields
		BEGIN
			SET @EvaluationExpression = 'EXISTS(SELECT 1 FROM PMPERMIT WHERE PMPERMITID = ' + '''' + @CaseID + ''' AND ' +  @Field + ' ' + @Operator + ' ''' + @FieldValue + ''')'
		...
		END					
		ELSE IF CHARINDEX(@CustomSaverPermitManagement, @TableName) > 0 --Custom Field evaluation
		BEGIN
		...
		BEGIN
			IF EXISTS (
			SELECT 1 FROM PMPERMITFEE INNER JOIN CACOMPUTEDFEE ON PMPERMITFEE.CACOMPUTEDFEEID = CACOMPUTEDFEE.CACOMPUTEDFEEID
            WHERE PMPERMITFEE.PMPERMITID = @CaseID AND 
			...
			BEGIN
				SET @EvaluationExpression = 'EXISTS	(SELECT 1 FROM (' + 'SELECT (SUM(CACOMPUTEDFEE.COMPUTEDAMOUNT) - SUM(CACOMPUTEDFEE.AMOUNTPAIDTODATE)) AS ' + @ColumnName +
					' FROM CACOMPUTEDFEE WHERE CACOMPUTEDFEE.CACOMPUTEDFEEID IN (SELECT PMPERMITFEE.CACOMPUTEDFEEID FROM PMPERMITFEE WHERE PMPERMITID = ''' + @CaseID + ''')' +
