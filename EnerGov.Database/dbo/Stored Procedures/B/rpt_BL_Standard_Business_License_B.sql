-- exec rpt_BL_Standard_Business_License_B '0ef9a8ed-c48b-435b-a94c-4a1e5bac9430',''
CREATE PROCEDURE rpt_BL_Standard_Business_License_B
...
SELECT BL.BLLICENSEID, BL.LICENSENUMBER, CASE WHEN BL.EXPIRATIONDATE = '1/1/2999' THEN NULL ELSE BL.EXPIRATIONDATE END AS EXPIRATIONDATE, 
	...
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Name_B') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Name')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Name_B') END Municipality_Name,
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Address_B') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Address')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Address_B') END Municipality_Address,
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Phone_Number_B') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Phone_Number')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Municipality_Phone_Number_B') END Municipality_Phone_Number,
	(SELECT REPLACE(R.REPORTTEXT,CHAR(10),'<BR>') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Text1_B') Business_License_Text1_B,
	(SELECT REPLACE(R.REPORTTEXT,CHAR(10),'<BR>') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Text2_B') Business_License_Text2_B,
    (SELECT R.REPORTTEXT FROM RPTTEXTLIB R WHERE r.TEXTNAME = 'Business_License_Report_Title_B') Business_License_Report_Title_B,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R WHERE r.TEXTNAME = 'License_or_Permit_Number_Label_B') License_or_Permit_Number_Label_B,
	(SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Business_License_Signature_Line_B') Business_License_Signature_Line_B,
	(SELECT R.[IMAGE] FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'BL_Logo_CAN_be_used_with_signature_image_B') LOGO,
	(SELECT CASE WHEN R.[IMAGE] IS NULL  THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'BL_Logo_CAN_be_used_with_signature_image_B') SHOWLOGO,
	(SELECT R.[IMAGE] FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'BL_Watermark_CANNOT_use_with_signature_image_B') WATERMARK,
	(SELECT R.DIMENSIONS FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'BL_Watermark_CANNOT_use_with_signature_image_B') Watermark_Dimensions,
	(SELECT CASE WHEN R.[IMAGE] IS NULL  THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'BL_Watermark_CANNOT_use_with_signature_image_B') SHOWWATERMARK,
	(SELECT R.[IMAGE] FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'Business_License_Signature_Image_B') SIGIMAGE,
	(SELECT CASE WHEN R.[IMAGE] IS NULL  THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'Business_License_Signature_Image_B') SHOWSIGIMAGE,
