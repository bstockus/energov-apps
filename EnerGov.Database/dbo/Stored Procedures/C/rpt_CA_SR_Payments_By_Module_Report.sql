...
CREATE PROCEDURE [dbo].[rpt_CA_SR_Payments_By_Module_Report]
...
SELECT	CAComputedFee.CAComputedFeeID
		...
		,COALESCE(BLLICENSE.LICENSENUMBER, CODEVIO.CASENUMBER, CMCODECASE.CASENUMBER, ILLICENSE.LICENSENUMBER, PLAPPLICATION.APPNUMBER
		...
		,CACOMPUTEDFEE.FEEName AS FeeName
...
SELECT	CAMiscFee.CAMiscFeeID AS CAComputedFeeID
		,CATransaction.ReceiptNumber
		,CAInvoice.InvoiceNumber
		,'Misc Fee' AS CaseNumber
		,COALESCE(CATransactionMiscFee.PaidAmount, CATRANSACTIONPAYMENT.PAYMENTAMOUNT) AS PaidAmount
		,CATransactionMiscFee.REFUNDAMOUNT
		,CAMiscFee.Amount AS ComputedAmount
		,CATransactionPayment.PaymentDate
		,CAPaymentMethod.Name AS PaymentMethod
		,CATransactionPayment.SupplementalData
		,CATransactionType.Name AS TransactionType
		,CAMISCFEE.FEENAME AS FeeName
		,'Cashier' AS Module
		--,(SELECT R.[IMAGE] FROM RPTIMAGELIB R WHERE R.IMAGENAME = 'Municipality_Logo') LOGO
	    ,(SELECT CASE WHEN R.[IMAGE] IS NULL THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R WHERE R.IMAGENAME = 'Municipality_Logo') SHOWLOGO
	    ,(SELECT R.REPORTTEXT FROM RPTTEXTLIB R WHERE R.TEXTNAME = 'Municipality_Name') Municipality_Name
...
WHERE CATransactionPayment.PaymentDate BETWEEN @STARTDATE AND @ENDDATE AND 
