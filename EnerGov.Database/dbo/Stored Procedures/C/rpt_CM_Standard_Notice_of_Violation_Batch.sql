...
/*
exec rpt_CM_Standard_Notice_of_Violation_Batch '20160101', '20180101', '0D3D4CF9-F10F-4AAD-B782-2E8C6F9CFF6D'
SELECT RPTREPORTID FROM RPTREPORT R WHERE R.REPORTNAME = 'CM_Standard_Notice_of_Violation_Batch.rpt'
*/


CREATE PROCEDURE rpt_CM_Standard_Notice_of_Violation_Batch
@STARTDATE DATETIME,
@ENDDATE DATETIME,
@REPORTID VARCHAR(36)
AS
...
SELECT CM.CMCODECASEID, CM.CASENUMBER, CM.OPENEDDATE,
	...
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Municipality_Name') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Name')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Municipality_Name') END Municipality_Name,
	CASE WHEN (SELECT ISNULL(R.REPORTTEXT,'') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Municipality_Address') = '' 
		THEN (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Municipality_Address')
		ELSE (SELECT R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Municipality_Address') END Municipality_Address,
	(SELECT  REPLACE(R.REPORTTEXT,CHAR(10),'<BR>') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Text1') Notice_Of_Violation_Text1,
	(SELECT  REPLACE(R.REPORTTEXT,CHAR(10),'<BR>') FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Text2') Notice_Of_Violation_Text2,
	(SELECT  R.REPORTTEXT FROM RPTTEXTLIB R
		WHERE R.TEXTNAME = 'Notice_Of_Violation_Title') Notice_Of_Violation_Title,
	--(SELECT R.[IMAGE] FROM RPTIMAGELIB R
	--	WHERE R.IMAGENAME = 'Code_Case_Logo') LOGO,
	(SELECT CASE WHEN R.[IMAGE] IS NULL  THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R
		WHERE R.IMAGENAME = 'Code_Case_Logo') SHOWLOGO,
