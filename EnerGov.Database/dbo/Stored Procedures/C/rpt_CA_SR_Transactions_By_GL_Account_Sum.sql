...
CREATE PROCEDURE [dbo].[rpt_CA_SR_Transactions_By_GL_Account_Sum]
@STARTDATE AS DATETIME,
@ENDDATE AS DATETIME,
@ACCOUNT AS VARCHAR(36)
AS
BEGIN
...
SELECT	TB.GLACCOUNTNAME, UPPER(TB.PaymentMethod) AS PaymentMethod, SUM(ISNULL(TB.PAIDAMOUNT,0)) AS PAIDAMOUNT, SUM(ISNULL(TB.POSTINGAMOUNT,0)) AS POSTINGAMOUNT
FROM (

SELECT	CASE WHEN (SELECT COALESCE(S.BITVALUE,0) FROM SETTINGS S WHERE S.NAME = 'EnableTCIntegration') = 0 THEN GLACCOUNT.NAME ELSE CAFEE.ARFEECODE END AS GLACCOUNTNAME
		, CAPAYMENTMETHOD.NAME AS PaymentMethod, (CATRANSACTIONFEE.PAIDAMOUNT - 2*CATRANSACTIONFEE.REFUNDAMOUNT) AS PAIDAMOUNT,
...
AND   (COALESCE(CATRANSACTIONGLPOSTING.CREDITACCOUNTID,CAFEE.ARFEECODE) = @ACCOUNT)

UNION ALL

SELECT	CASE WHEN (SELECT COALESCE(S.BITVALUE,0) FROM SETTINGS S WHERE S.NAME = 'EnableTCIntegration') = 0 THEN GLACCOUNT.NAME ELSE CAFEE.ARFEECODE END AS GLACCOUNTNAME
		, CAPAYMENTMETHOD.NAME AS PaymentMethod, (CATRANSACTIONMISCFEE.PAIDAMOUNT - 2*CATRANSACTIONMISCFEE.REFUNDAMOUNT) AS PAIDAMOUNT,
		CASE WHEN (SELECT COALESCE(S.BITVALUE,0) FROM SETTINGS S WHERE S.NAME = 'EnableTCIntegration') = 0 THEN (
		CATRANSACTIONGLPOSTING.POSTINGAMOUNT * CASE WHEN CATRANSACTIONTYPE.NAME = 'Refund' THEN -1 ELSE 1 END)
...
GROUP BY TB.GLACCOUNTNAME, TB.PaymentMethod
