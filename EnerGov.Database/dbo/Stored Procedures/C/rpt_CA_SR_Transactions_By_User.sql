...
CREATE PROCEDURE [dbo].[rpt_CA_SR_Transactions_By_User]
...
SELECT CACOMPUTEDFEE.CACOMPUTEDFEEID, CACOMPUTEDFEE.FEENAME, CACOMPUTEDFEE.COMPUTEDAMOUNT, CATRANSACTIONFEE.PAIDAMOUNT, CATRANSACTIONFEE.REFUNDAMOUNT, 
       USERS.ID, USERS.SUSERGUID, 
	   CASE WHEN CATRANSACTIONPAYMENT.ISONLINE = 1 THEN 'Online Payment' ELSE USERS.FNAME + ' ' + USERS.LNAME END as USERNAME,
	   CAINVOICE.INVOICENUMBER, CATRANSACTION.TRANSACTIONDATE, CAPAYMENTMETHOD.NAME AS PaymentMethod, 
	   ...
	   COALESCE (BLLICENSE.LICENSENUMBER, CMCODECASE.CASENUMBER, CODEVIO.CASENUMBER, ILLICENSE.LICENSENUMBER, 
		PLAPPLICATION.APPNUMBER, PLPLAN.PLANNUMBER, PMPERMIT.PERMITNUMBER, PRPROJECT.PROJECTNUMBER, 
        RPLANDLORDLICENSE.LANDLORDNUMBER, RPPROPERTY.PROPERTYNUMBER, TXREMITTANCEACCOUNT.REMITTANCEACCOUNTNUMBER, IPCASE.CASENUMBER) AS CaseNumber
...
SELECT CAMiscFee.CAMiscFeeID AS CACOMPUTEDFEEID, CAMiscFee.FeeName, CAMiscFee.Amount AS COMPUTEDAMOUNT, CATransactionMiscFee.PaidAmount, CATRANSACTIONMISCFEE.REFUNDAMOUNT, 
	   ...
	   CASE WHEN CATRANSACTIONPAYMENT.ISONLINE = 1 THEN 'Online Payment' ELSE USERS.FNAME + ' ' + USERS.LNAME END as USERNAME, 
	   CAInvoice.InvoiceNumber, CATransaction.TransactionDate, CAPaymentMethod.Name AS PaymentMethod, 
       CATransactionPayment.SupplementalData, CATransactionType.Name AS TransactionType, CATransactionStatus.Name AS TransactionStatus, 'Misc Fee' as CaseNumber
		--,(SELECT R.[IMAGE] FROM RPTIMAGELIB R WHERE R.IMAGENAME = 'Municipality_Logo') LOGO
	    ,(SELECT CASE WHEN R.[IMAGE] IS NULL THEN 'N' ELSE 'Y' END FROM RPTIMAGELIB R WHERE R.IMAGENAME = 'Municipality_Logo') SHOWLOGO
	    ,(SELECT R.REPORTTEXT FROM RPTTEXTLIB R WHERE R.TEXTNAME = 'Municipality_Name') Municipality_Name
	    ,(SELECT R.REPORTTEXT FROM RPTTEXTLIB R WHERE R.TEXTNAME = 'Municipality_Page_Footer') Municipality_Page_Footer
