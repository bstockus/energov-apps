CREATE PROCEDURE [integration].[USP_GET_GL_POSTING_BY_DATE] 
(
	...
	@includeRefunds BIT, 
	@includeNSFs BIT, 
	@includeOnlinePayments BIT, 
	@includeMiscFees BIT, 
	@includeEscrowAccountTransactions BIT
)
AS
BEGIN
...
WHERE ([CATRANSACTIONGLPOSTING].[POSTINGDATE] >= @postingStartDate) 
AND ([CATRANSACTIONGLPOSTING].[POSTINGDATE] <= @postingEndDate) 
AND ([CATRANSACTIONGLPOSTING].[EXPORTEDON] IS NULL OR @includeExportedPreviously = 1) 
AND (((2 <> [CATRANSACTION].[CATRANSACTIONSTATUSID]) AND (6 <> [CATRANSACTION].[CATRANSACTIONTYPEID])) OR (@includeVoids = 1)) 
AND ((4 <> [CATRANSACTION].[CATRANSACTIONTYPEID]) OR (@includeRefunds = 1)) 
AND ((5 <> [CATRANSACTION].[CATRANSACTIONTYPEID]) OR (@includeNSFs = 1)) 
AND ([CATRANSACTIONPAYMENT].[ISONLINE] = 0 OR @includeOnlinePayments = 1)  
...
AND ((5 <> [CATRANSACTION].[CATRANSACTIONTYPEID]) OR (@includeNSFs = 1)) 
AND ([CATRANSACTIONPAYMENT].[ISONLINE] = 0 OR @includeOnlinePayments = 1)  
AND @includeEscrowAccountTransactions = 1
