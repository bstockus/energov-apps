CREATE PROCEDURE [common].[USP_HISTORYSYSTEMSETUP_GETBYFORMID]
(	
	@PAGE_NUMBER AS INT	= 1,
	@PAGE_SIZE AS INT =	10,
	@IS_ASCENDING AS BIT =	1,
	@FORMID AS CHAR(36),
	@ACTIONLIST AS [dbo].[IntegerIds] READONLY,
	...
	@RECORDNAMELIST AS [dbo].[HISTORYSYSTEMSETUP_COLUMNFILTERS] READONLY,
...
WITH RAW_DATA AS (
	SELECT
		...
		[dbo].[HISTORYSYSTEMSETUP].[FORMID],
		[dbo].[HISTORYSYSTEMSETUP].[RECORDNAME],
		[dbo].[USERS].[LNAME],
		[dbo].[USERS].[FNAME],
		[dbo].[HISTORYSYSTEMSETUP].[ACTION],
		CASE
				 ...
				 (
					ORDER BY
						CASE
							...
							WHEN @SORTBYCOLUMNNAME = 'RECORDNAME' THEN [dbo].[HISTORYSYSTEMSETUP].[RECORDNAME]
							...
							WHEN @SORTBYCOLUMNNAME = 'ACTIONTYPE' THEN CONVERT(NVARCHAR(MAX), [dbo].[HISTORYSYSTEMSETUP].[ACTION])
						END
					ASC,
	...
	WHERE [dbo].[HISTORYSYSTEMSETUP].[FORMID] = @FORMID
			AND ((SELECT COUNT(1) FROM @ACTIONLIST) = 0  OR [dbo].[HISTORYSYSTEMSETUP].[ACTION] IN (SELECT [ID] FROM @ACTIONLIST))
			AND ((SELECT COUNT(1) FROM @CHANGEDBYLIST) = 0  OR [dbo].[HISTORYSYSTEMSETUP].[CHANGEDBY] IN (SELECT [RECORDID] FROM @CHANGEDBYLIST))
			AND ((SELECT COUNT(1) FROM @RECORDNAMELIST) = 0  OR [dbo].[HISTORYSYSTEMSETUP].[RECORDNAME] IN (SELECT [FILTERVALUES] FROM @RECORDNAMELIST))
